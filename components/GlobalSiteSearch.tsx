...
export function GlobalSiteSearch() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      if (trimmed.length < 2) {
        setResults([]);
        setStatus("Type at least two letters.");
        return;
      }
      ...
      try {
        ...
        if (response.status === 403) {
          setResults([]);
          setStatus("The reveal must be open before its contents can be searched.");
          return;
        }
        ...
        if ((error as Error).name !== "AbortError") {
          setResults([]);
          setStatus("Search needs another moment. Please try again.");
        }
      }
    }, trimmed.length < 2 ? 0 : 260);
