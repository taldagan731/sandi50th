  ...
  return (
    <section ref={galleryRef} className={`${styles.chapterFilmSection} ${rowClass} ${filmMotion.moving ? styles.motionEnabled : ""} ${paused ? styles.marqueePaused : ""}`} aria-labelledby={`chapter-film-title-${chapterNumber}`} onPointerDown={event => { if (event.pointerType !== "mouse") setTouchPaused(true); }}>
                    ...
                    <div className={styles.marqueeGroup} aria-hidden={duplicate || undefined} key={duplicate ? "duplicate" : "original"}>
                      {row.map(photo => (
                        <button className={styles.marqueePhoto} type="button" key={photo.id + (duplicate ? "-duplicate" : "")} tabIndex={duplicate ? -1 : undefined} onPointerUp={event => { if (event.pointerType !== "mouse") { event.preventDefault(); setExpanded(photo); } }} onClick={() => setExpanded(photo)}>
                          <Image {...filmPhotoProps(photo.id)} width={640} height={480} sizes="(max-width: 700px) 46vw, 280px" quality={68} data-reveal-photo="true" data-media-id={photo.id} alt={duplicate ? "" : photo.caption || `Photograph from ${chapterTitle} shared by ${photo.contributorName}`} loading="lazy" />
      ...
      <div className={styles.marqueeReducedGrid} aria-label={`Photograph album for ${chapterTitle}`}>
        {ordered.map(photo => <button type="button" key={photo.id} onPointerUp={event => { if (event.pointerType !== "mouse") { event.preventDefault(); setExpanded(photo); } }} onClick={() => setExpanded(photo)}><Image src={`${mediaUrl(photo.id)}?poster=1`} width={640} height={480} sizes="(max-width: 700px) 46vw, 280px" quality={68} data-reveal-photo="true" data-media-id={photo.id} alt={photo.caption || `Photograph from ${chapterTitle} shared by ${photo.contributorName}`} loading="lazy" /></button>)}
      </div>
