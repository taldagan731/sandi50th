...
const filmPhotoProps = (id: string) => ({
  src: `${mediaUrl(id)}?poster=1`,
});

export function ChildhoodCylinder({ photos, chapterPhotos }: { photos: Photo[]; chapterPhotos: Photo[] }) {
  const selected = useMemo(() => {
    const byId = new Map(photos.map(photo => [photo.id, photo]));
    const approved = approvedIds.flatMap(id => byId.get(id) ? [byId.get(id)!] : []);
    const approvedSet = new Set(approved.map(photo => photo.id));
    const replacements = [...chapterPhotos]
      .filter(photo => !approvedSet.has(photo.id))
      .sort((a, b) => (a.yearStart ?? 9999) - (b.yearStart ?? 9999) || a.displayOrder - b.displayOrder);
    return [...approved, ...replacements].slice(0, 18);
  }, [photos, chapterPhotos]);
  const gallery = useMemo(() => [...chapterPhotos]
    .filter(photo => !selected.some(selectedPhoto => selectedPhoto.id === photo.id))
    .sort((a, b) => (a.yearStart ?? 9999) - (b.yearStart ?? 9999) || a.displayOrder - b.displayOrder)
    .slice(0, 59), [chapterPhotos, selected]);
    ...
    <section className={styles.section} aria-labelledby="childhood-cylinder-title">
      ...
      <div className={`${styles.stage} ${paused ? styles.paused : ""}`} tabIndex={0} role="region" aria-label="Childhood photograph cylinder" onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} onMouseEnter={() => setPaused(true)} onMouseLeave={() => { if (!pointer.current.active) setPaused(false); }} onKeyDown={event => { if (event.key === "ArrowLeft" || event.key === "ArrowRight") { event.preventDefault(); setDragAngle(angle => angle + (event.key === "ArrowLeft" ? 20 : -20)); } }}>
        <div className={styles.drift}><div className={styles.ring} style={{ "--drag-angle": `${dragAngle}deg` } as CSSProperties}>
          {selected.map((photo, index) => <button key={photo.id} type="button" className={styles.face} style={{ "--face-angle": `${index * (360 / selected.length)}deg` } as CSSProperties} onPointerUp={event => { if (event.pointerType !== "mouse") { event.preventDefault(); open(photo); } }} onClick={() => open(photo)}><Image {...filmPhotoProps(photo.id)} width={640} height={640} sizes="(max-width: 700px) 38vw, 220px" quality={68} data-reveal-photo="true" data-media-id={photo.id} alt={photo.caption || `Childhood photograph of Sandi shared by ${photo.contributorName}`} draggable={false} loading="lazy" /><span>{photo.caption || photo.originalName}</span></button>)}
        </div></div>
      </div>
      <div className={styles.reducedGrid}>{selected.map(photo => <button type="button" key={photo.id} onPointerUp={event => { if (event.pointerType !== "mouse") { event.preventDefault(); open(photo); } }} onClick={() => open(photo)}><Image src={`${mediaUrl(photo.id)}?poster=1`} width={640} height={640} sizes="(max-width: 700px) 46vw, 280px" quality={68} data-reveal-photo="true" data-media-id={photo.id} alt={photo.caption || `Childhood photograph of Sandi shared by ${photo.contributorName}`} loading="lazy" /></button>)}</div>
      <section
        ref={galleryRef}
      ...
      >
                    ...
                    <div className={styles.marqueeGroup} aria-hidden={duplicate || undefined} key={duplicate ? "duplicate" : "original"}>
                      {row.map(photo => (
                        <button className={styles.marqueePhoto} type="button" key={photo.id + (duplicate ? "-duplicate" : "")} tabIndex={duplicate ? -1 : undefined} onPointerUp={event => { if (event.pointerType !== "mouse") { event.preventDefault(); open(photo); } }} onClick={() => open(photo)}>
                          <Image {...filmPhotoProps(photo.id)} width={640} height={480} sizes="(max-width: 700px) 46vw, 280px" quality={68} data-reveal-photo="true" data-media-id={photo.id} alt={duplicate ? "" : photo.caption || `Photograph of Sandi shared by ${photo.contributorName}`} loading="lazy" />
        ...
        <div className={styles.marqueeReducedGrid} aria-label="Childhood photograph album">
          {gallery.map(photo => <button type="button" key={photo.id} onPointerUp={event => { if (event.pointerType !== "mouse") { event.preventDefault(); open(photo); } }} onClick={() => open(photo)}><Image src={`${mediaUrl(photo.id)}?poster=1`} width={640} height={480} sizes="(max-width: 700px) 46vw, 280px" quality={68} data-reveal-photo="true" data-media-id={photo.id} alt={photo.caption || `Photograph of Sandi shared by ${photo.contributorName}`} loading="lazy" /></button>)}
        </div>
      </section>
