  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const payload = {
        ...
        files: files.map(item => ({ name: item.file.name, type: item.file.type, size: item.file.size }))
      };
      ...
      for (let index = 0; index < prepared.uploads.length; index += 1) {
        ...
        const { error } = await supabase.storage
          .from("sandi-memories")
          .uploadToSignedUrl(target.path, target.token, selected, { contentType: selected.type, upsert: false });
        if (error) throw error;
        completedFiles.push({ path: target.path, name: selected.name, type: selected.type, size: selected.size });
  ...
  if (submitted) {
    return (
      <section className="contributionSuccess" aria-live="polite">
        ...
        <p>
          Your contribution has been received securely. The organizers can now review it for Sandi’s private film and archive.
        </p>
      ...
      {opened && (
        <div className="contributionColumns">
          <section className="panel formDetails">
            ...
            {uploading && <div className="uploadProgress" role="status"><span style={{ width: `${files.length ? (uploadStep / files.length) * 100 : 25}%` }} /><p>{files.length ? `Uploading item ${Math.min(uploadStep + 1, files.length)} of ${files.length}…` : "Saving your written memory…"}</p></div>}
            {uploadError && <p className="uploadError" role="alert">{uploadError}</p>}
            <button className="primary submitMemory" type="submit" disabled={uploading}>{uploading ? "Preserving your memory…" : "Send my contribution securely"}</button>
            <p className="secureNote">Files are sent directly to private cloud storage. Please keep this page open until confirmation appears.</p>
          </section>
