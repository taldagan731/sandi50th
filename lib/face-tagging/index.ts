...
type FaceTagRow = Record<string, unknown> & { id?: string; media_asset_id: string; person_name: string; status: string; source?: string; confidence?: number | null };

async function allFaceTags(projectId: string, columns: string) {
  const supabase = createAdminClient();
  const rows: FaceTagRow[] = [];
  const pageSize = 1000;
  for (let from = 0; ; from += pageSize) {
    const { data, error } = await supabase.from("photo_face_tags").select(columns).eq("project_id", projectId).range(from, from + pageSize - 1);
    if (error) throw error;
    const page = (data ?? []) as unknown as FaceTagRow[];
    rows.push(...page);
...
export async function faceTaggingStatus(projectId: string) {
  ...
  let data: FaceTagRow[];
  try {
    data = await allFaceTags(projectId, "media_asset_id,status,person_name,source,confidence");
  } catch (error) {
    if (migrationMissing(error as { code?: string; message?: string })) return { migrationRequired: true, total: photos.length, scanned: 0, remaining: photos.length, confirmed: 0, questions: 0, people: 0 };
    throw error;
  }
...
export async function processNextFacePhoto(projectId: string, options: { dryRun?: boolean } = {}) {
  const supabase = createAdminClient();
  const photos = await photoInventory(projectId);
  const allTags = await allFaceTags(projectId, "id,media_asset_id,person_name,x,y,width,height,status");
  ...
  const { data: existingTarget } = await supabase.from("photo_face_tags")
    .select("id")
    .eq("project_id", projectId)
    .eq("media_asset_id", target.id)
    .limit(1)
    .maybeSingle();
  if (existingTarget) return { complete: false, skippedExisting: true, mediaId: target.id, inserted: 0 };
  if (rows.length) {
