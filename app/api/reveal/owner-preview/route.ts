...
export async function GET(request: Request) {
  ...
  const token = invited
    ? createRevealPreviewToken(OWNER_INVITATION_EXPIRES)
    : { expires: Number(expires), signature: signature! };
  const requestedNext = url.searchParams.get("next") || "/reveal";
  const next = requestedNext === "/reveal" || requestedNext.startsWith("/reveal?")
    ? requestedNext
    : "/reveal";
  const maxAge = Math.max(1, token.expires - Math.floor(Date.now() / 1000));
  const response = NextResponse.redirect(new URL(next, url.origin));
  response.cookies.set(
    REVEAL_PREVIEW_COOKIE,
    `${token.expires}.${token.signature}`,
    revealPreviewCookieOptions(maxAge)
  );
